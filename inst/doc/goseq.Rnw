...
\np
The output of nullp contains all the information supplied to the nullp function as well as the estimated PWF.  This saves us having to re-enter the same information for the next step of the analysis.
...
The resulting object is ordered by GO category over representation amongst DE genes.  Note that we did not have to re-enter the genome as "hg18" or gene identifier as "ensGene" as this information is retained in the pwf object.
...
GO.samp=goseq(pwf,method="Sampling",repcnt=1000)
@
<<head_samp>>=
head(GO.samp)
@
...
<<GO.nobias>>=
GO.nobias=goseq(pwf,method="Hypergeometric")
head(GO.nobias)
@
...
<<GO.limited>>=
GO.MF=goseq(pwf,test.cats=c("GO:MF"))
head(GO.MF)
@
...
head(len)
@
After some data cleanup, the length data and the DE data is then passed to the \texttt{makespline} function to produce the PWF.  The \texttt{nullp} usually returns a list containing three objects named "pwf", "genome" and "id".  The data frame has 3 columns, the original DEgenes vector, the length bias data (in a column called bias.data) and the PWF itself (in a column named pwf).  The names of the genes are also kept in this data frame as the names of the rows.  The "genome" and "id" entries contain the values provided to the genome and id arguments of the \texttt{nullp} function.  If these entries are not given (as is the case when the user supplies the length data) \texttt{nullp} just returns the data frame described above.
\np
...
Note that the first thing the getgo function does is to convert the UCSC genome/ID namings into the naming convention used by the organism packages.  This is done using two hard coded conversion vectors that are included in the \texttt{goseq} package but usually hiden from the user.
...
This is shown as an illustration of how to supply your own mapping between gene ID and category, KEGG analysis is actually natively supported by GOseq and we could have performed it with the following code.

<<KEGG_goseq>>=
pwf=nullp(genes,'hg18','ensGene')
KEGG=goseq(pwf,test.cats="KEGG")
head(KEGG)
@
...
To use the count bias when doing GO analysis, simply pass this vector to \texttt{nullp} using the \texttt{bias.data} option.  Note that we have to supply "hg18" and "ensGene" to \texttt{goseq} as it is not used by nullp and hence not in the pwf.counts object.
