...
In order to allow for automatic data retrieval, the user has to tell \texttt{goseq} what genome and gene ID format were used to summarize the data.  In our case we will use GRCh37 build of the human genome, we check what code this corresponds to by running:
