...
Which lists the genome codes in the far left column, headed ``Genome''.  As we are using ``hg19".  We also know that we used ENSEMBL Gene ID to summarize our read data, we check what code this corresponds to by running:
